...
app = Flask(__name__)
CORS(app)  # Allow requests from any origin (not recommended for production)
# --------------------------------------------------------------------------------------------------------------
...
def search_leetcode_questions():
    query_string = request.args.get('query', '')
    query_terms = [term.lower() for term in query_string.strip().split()]

    # Call the function to calculate and get the top 20 links and names
    top_questions = calculate_sorted_order_of_documents(query_terms)

    # Prepare the response in JSON format
    response = jsonify(top_questions)
    return response
